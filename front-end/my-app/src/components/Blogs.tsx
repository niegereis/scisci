        ...
        >          
          {blogs.data.map((blog) => (
            <Link key={blog.id} href={`/blog/${blog.id}`}>
              <article className="bg-white rounded-xl overflow-hidden drop-shadow-md">
                <img
                ...
                />
                <div className="p-8">
                  <h3 className="font-bold text-2xl my-1">{blog.articleTitle}</h3>
                  <p className="text-gray-600 text-xl">{blog.blogDescripition}</p>
                </div>
              </article>
            </Link>
          ))}
